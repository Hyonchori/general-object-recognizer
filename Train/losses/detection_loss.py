...
from ..train_utils import bboxes_iou


class DetectionLosses:
    def __init__(
            self,
            num_classes: int = 80,  # COCO dataset classes
            use_l1: bool = False
    ):
    ...
    def __call__(
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                gt_bboxes_per_image = labels[batch_idx, :num_gt, 0:4]
                ...
                reg_target = gt_bboxes_per_image[matched_gt_indices]
                if self.use_l1:
                    l1_target = self.get_l1_target(
                        outputs.new_zeros((num_fg_img, 4)),
                        gt_bboxes_per_image[matched_gt_indices],
    ...
    def get_assignments(
        ...
        if mode == "cpu":
            ...
            matched_gt_indices = matched_gt_indices.cuda()
    ...
    def get_in_boxes_info(
        ...
        gt_bboxes_per_image_clip[:, 0] -= gt_bboxes_per_image[:, 2] * 0.5
        gt_bboxes_per_image_clip[:, 1] -= gt_bboxes_per_image[:, 3] * 0.5
    ...
    def dynamic_k_matching(cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        matched_gt_indices = matching_matrix[:, fg_mask_inboxes].argmax(0)
        gt_matched_classes = gt_classes[matched_gt_indices]

        pred_ious_this_matching = (matching_matrix * pair_wise_ious).sum(0)[
            fg_mask_inboxes
        ]
        return num_fg, gt_matched_classes, pred_ious_this_matching, matched_gt_indices
